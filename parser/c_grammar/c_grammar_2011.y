...
%type <word> STRING_LITERAL
%type <word> TYPEDEF_NAME
%type <word> FUNC_NAME
%type <number> I_CONSTANT
%type <number> F_CONSTANT
...
primary_expression
	: IDENTIFIER {$$ = rule.primary_expression__IDENTIFIER($1);}
	| constant {$$ = rule.primary_expression__constant();}
	| string {$$ = rule.primary_expression__string();}
	| '(' expression ')' {$$ = rule.primary_expression__LP_expression_RP($2);}
	| generic_selection {$$ = rule.primary_expression__generic_selection($1);}
	;

constant
	: I_CONSTANT {$$ = rule.constant__I_CONSTANT($1);}
	| F_CONSTANT {$$ = rule.constant__F_CONSTANT($1);}
...
string
	: STRING_LITERAL {$$ = rule.string__STRING_LITERAL($1);}
	| FUNC_NAME {$$ = rule.string__FUNC_NAME($1);}
	;

generic_selection
	: GENERIC '(' assignment_expression ',' generic_assoc_list ')' {$$ = rule.generic_selection__GENERIC_LP_assignment_expression_COMMA_generic_assoc_list_RP($3, $5);}
	;

generic_assoc_list
	: generic_association {$$ = rule.generic_assoc_list__generic_association($1);}
	| generic_assoc_list ',' generic_association {$$ = rule.generic_assoc_list__generic_assoc_list_COMMA_generic_association($1, $3);}
	;

generic_association
	: type_name ':' assignment_expression {$$ = rule.generic_association__type_name_COLON_assignment_expression($1, $3);}
	| DEFAULT ':' assignment_expression {$$ = rule.generic_association__DEFAULT_COLON_assignment_expression($3);}
	;

postfix_expression
	: primary_expression {$$ = rule.postfix_expression__primary_expression($1);}
	| postfix_expression '[' expression ']' {$$ = rule.postfix_expression__postfix_expression_LSB_expression_RSB($1, $3);}
	| postfix_expression '(' ')' {$$ = rule.postfix_expression__postfix_expression_LP_RP($1);}
	| postfix_expression '(' argument_expression_list ')' {$$ = rule.postfix_expression__postfix_expression_LP_argument_expression_list_RP($1, $3);}
...
declaration
	: declaration_specifiers ';' {$$ = rule.declaration__declaration_specifiers_SCOLON($1);}
	| declaration_specifiers init_declarator_list ';' {$$ = rule.declaration__declaration_specifiers_init_declarator_list_SCOLON($1, $2);}
	| static_assert_declaration {$$ = rule.declaration__static_assert_declaration($1);}
	;
...
declaration_specifiers
	...
	| type_specifier {$$ = rule.declaration_specifiers__type_specifier($1);}
...
init_declarator_list
	: init_declarator {$$ = rule.init_declarator_list__init_declarator($1);}
	| init_declarator_list ',' init_declarator {$$ = rule.init_declarator_list__init_declarator_list_COMMA_init_declarator($1, $3);}
	;

init_declarator
	: declarator '=' initializer {$$ = rule.init_declarator__declarator_EQ_initializer($1, $3);}
	| declarator {$$ = rule.init_declarator__declarator($1);}
	;
...
type_specifier
	...
	| atomic_type_specifier {$$ = rule.type_specifier__atomic_type_specifier($1);}
	| struct_or_union_specifier {$$ = rule.type_specifier__struct_or_union_specifier($1);}
	| enum_specifier {$$ = rule.type_specifier__enum_specifier($1);}
	| TYPEDEF_NAME {$$ = rule.type_specifier__TYPEDEF_NAME($1);}
	;
...
declarator
	: pointer direct_declarator {$$ = rule.declarator__pointer_direct_declarator($1, $2);}
	| direct_declarator {$$ = rule.declarator__direct_declarator($1);}
	;

direct_declarator
	: IDENTIFIER {$$ = rule.direct_declarator__IDENTIFIER($1);}
	| '(' declarator ')' {$$ = rule.direct_declarator__LP_declarator_RP($2);}
	| direct_declarator '[' ']' {$$ = rule.direct_declarator__direct_declarator_LSB_RSB($1);}
	| direct_declarator '[' '*' ']' {$$ = rule.direct_declarator__direct_declarator_LSB_MULT_RSB($1);}
	| direct_declarator '[' STATIC type_qualifier_list assignment_expression ']' {$$ = rule.direct_declarator__direct_declarator_LSB_STATIC_type_qualifier_list_assignment_expression_RSB($1, $4, $5);}
	| direct_declarator '[' STATIC assignment_expression ']' {$$ = rule.direct_declarator__direct_declarator_LSB_STATIC_assignment_expression_RSB($1, $4);}
	| direct_declarator '[' type_qualifier_list '*' ']' {$$ = rule.direct_declarator__direct_declarator_LSB_type_qualifier_list_MULT_RSB($1, $3);}
	| direct_declarator '[' type_qualifier_list STATIC assignment_expression ']' {$$ = rule.direct_declarator__direct_declarator_LSB_type_qualifier_list_STATIC_assignment_expression_RSB($1, $3, $5);}
	| direct_declarator '[' type_qualifier_list assignment_expression ']' {$$ = rule.direct_declarator__direct_declarator_LSB_type_qualifier_list_assignment_expression_RSB($1, $3, $4);}
	| direct_declarator '[' type_qualifier_list ']' {$$ = rule.direct_declarator__direct_declarator_LSB_type_qualifier_list_RSB($1, $3);}
	| direct_declarator '[' assignment_expression ']' {$$ = rule.direct_declarator__direct_declarator_LSB_assignment_expression_RSB($1, $3);}
	| direct_declarator '(' parameter_type_list ')' {$$ = rule.direct_declarator__direct_declarator_LP_parameter_type_list_RP($1, $3);}
	| direct_declarator '(' ')' {$$ = rule.direct_declarator__direct_declarator_LP_RP($1);}
	| direct_declarator '(' identifier_list ')' {$$ = rule.direct_declarator__direct_declarator_LP_identifier_list_RP($1, $3);}
	;

pointer
	: '*' type_qualifier_list pointer {$$ = rule.pointer__MULT_type_qualifier_list_pointer($2, $3);}
	| '*' type_qualifier_list {$$ = rule.pointer__MULT_type_qualifier_list($2);}
	| '*' pointer {$$ = rule.pointer__MULT_pointer($2);}
	| '*' {$$ = rule.pointer__MULT();}
	;

type_qualifier_list
	: type_qualifier {$$ = rule.type_qualifier_list__type_qualifier($1);}
	| type_qualifier_list type_qualifier {$$ = rule.type_qualifier_list__type_qualifier_list_type_qualifier($1, $2);}
	;


parameter_type_list
	: parameter_list ',' ELLIPSIS {$$ = rule.parameter_type_list__parameter_list_COMMA_ELLIPSIS($1);}
	| parameter_list {$$ = rule.parameter_type_list__parameter_list($1);}
	;

parameter_list
	: parameter_declaration {$$ = rule.parameter_list__parameter_declaration($1);}
	| parameter_list ',' parameter_declaration {$$ = rule.parameter_list__parameter_list_COMMA_parameter_declaration($1, $3);}
	;
...
translation_unit
	: external_declaration {$$ = rule.translation_unit__external_declaration($1);}
	| translation_unit external_declaration {$$ = rule.translation_unit__translation_unit_external_declaration($1, $2);}
	;

external_declaration
	: function_definition {$$ = rule.external_declaration__function_definition($1);}
	| declaration {$$ = rule.external_declaration__declaration($1);}
	;
